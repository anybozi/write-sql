...
- **多指标命中**：扫描本表时列出用户需求中的**全部**命中项（精确 / 同义 / 疑似），禁止只取第一个；再并行打开各 `metric_file`。流程见 `SKILL.md`「多指标编排路径」。
- 用户同时要多个标准指标 + 非指标度量（如出账收入）时：标准指标走本索引；非指标度量在方案确认中记为「附加度量」，不写入本表。
- 标准指标命中后，以单指标文件中的技术口径 SQL 为权威；不要用经验路由改写标准指标 SQL；**多指标时禁止把各指标 WHERE 直接 AND 合并**，应条件分列（`CASE WHEN`）或按 `SKILL.md` 编排类型处理。
...
| “主宽入网按竣工时间 / 202605 按竣工” | 是 | 命中 M-BASIC-BB-001；口语「竣工」→ 069 `open_date`，不必追问 `subs_stat_date` |
| “按营服统计主宽和 129+ 宽带入网” | 是（多指标） | 命中 M-BASIC-BB-001 + M-BASIC-BB-004；走多指标编排，A 类同表条件聚合；可参考 VC-20260427-001 |
