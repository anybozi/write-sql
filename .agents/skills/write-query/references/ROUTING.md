...
| 号码清单补指定地址层级 | 附件号码 -> 069 -> 079 | `scenarios/SC-006_号码清单补地址层级.md` |
| 市场化合同有效揽装人、无号码收入网点诊断 | 110 -> 113/112/111 | `scenarios/SC-007_市场化合同有效揽装人.md` |
| 种子 serv_id + 拆机前月产品规格/附属产品属性宽表 | 用户种子 -> 069 月表 -> 105/106 月表 | `scenarios/SC-008_种子serv_id拆机前月属性宽表.md` |
| 实收 / 117；客户清单基本面·产数 / 047（含附件、圈定、直查） | 117 或 047；圈定常经 069/033 | `scenarios/SC-009_047117收入实收查询.md` |
| 揽装人认领规则调整责任田 | 069 -> 124 -> 125 | `scenarios/SC-010_揽装人认领责任田调整.md` |

> **专项场景单一入口**：上表是 `SC-001`~`SC-010` 的唯一指针表。下文「术语映射」「主表路由表」只保留 **主表 / 不要误选** 判断；补表步骤、JOIN、过程表血缘、风险审计与自检 **不再重复展开**，命中后只读对应 `SC-*.md`（或 `scenarios/INDEX.md`）。
...
| 竣工时间 | `subs_stat_date`（配 `subs_stat='301200'`） | 040/041/022 等订单表 | 订单状态变为竣工的日期 |
...
| 销售品发展量 / 订购动作 | 041 优惠订单表；历史月份用 `dwm_yz_rpt_comm_ba_msdisc_mon_final` | 020 销售品维表补编码/名称 | 燃气卫士、视联网、FTTR 等专项清单 | 动作事实在优惠订单表；当前表 `dwm_yz_rpt_comm_ba_msdisc_final` 只放当前套餐订单及历史未归档订单，查历史归档月必须用月表并锁 `par_month_id` |
