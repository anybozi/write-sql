...
> **专项场景单一入口**：上表是 `SC-001`~`SC-009` 的唯一指针表。下文「术语映射」「主表路由表」只保留 **主表 / 不要误选** 判断；补表步骤、JOIN、过程表血缘、风险审计与自检 **不再重复展开**，命中后只读对应 `SC-*.md`（或 `scenarios/INDEX.md`）。
...
| 7级地址 / 地址层级 / 装机地址上级地址 | 7 级/5 级地址 ID/名称 | 069 → 079 | - | 地址 ID 统一转字符；专项见 **§专项场景索引**（SC-006） |
